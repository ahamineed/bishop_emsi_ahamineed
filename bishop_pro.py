...
if st.session_state.verrouille:
    ...
    if st.button("Valider l'accès"):
        if mot_de_passe == "06101010":  # MODIFIE LE MOT DE PASSE ICI
...
def calcul_bishop_expert(params, couches, H, L, y_nappe, nappe_active, return_slices=False):
    xc, yc, R = params
    xs = np.linspace(0.05, L-0.05, 45)
    dx = xs[1] - xs[0]
    fs = 1.2
    slices_data = []
    for _ in range(25):
...
with st.sidebar:
    st.header("📐 Paramètres géométrique du talus")
...
if col_btn1.button("👁️ Afficher la Stratigraphie"):
    ...
    if erreurs:
        ...
        ax.set_aspect('equal')
        ax.set_xlim(-L*0.4, L*1.7); ax.set_ylim(-20, H+10)
...
if col_btn2.button("🚀 Lancer l'Analyse"):
    ...
    if erreurs:
        ...
        with st.spinner("Optimisation du cercle de rupture..."):
            ...
            ax.set_xlabel("Distance (m)"); ax.set_ylabel("Altitude (m)")
            ax.set_aspect('equal')
            ax.set_xlim(-L*0.4, L*1.7); ax.set_ylim(-20, H*1.3)
