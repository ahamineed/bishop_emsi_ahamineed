...
def valider_parametres():
    ...
    for i, d in enumerate(data_c):
        if d['ep'] <= 0: erreurs.append(f"Épaisseur de la {d['nom']} doit être positive.")
        if d['c'] < 0: erreurs.append(f"Cohésion de la {d['nom']} ne peut pas être négative.")
        if d['ph'] < 0: erreurs.append(f"Angle Phi de la {d['nom']} ne peut pas être négatif.")
        if d['ga'] <= 0: erreurs.append(f"Poids Gamma de la {d['nom']} doit être positif.")
    return erreurs
...
for i in range(nb_c):
    with cols_c[i]:
        st.subheader(f"Config. Couche {i+1}")
        nom_c = st.text_input(f"Nom de la couche", value=f"Couche {i+1}", key=f"nom_{i}")
        ...
        ga = st.number_input("Gamma (kN/m³)", value=19.0, key=f"ga_{i}")
...
if col_btn1.button("👁️ Vérifier Stratigraphie"):
    ...
    if erreurs:
        ...
        for c in couches:
            ax.fill_between([-L, L*2], c.y_bot, c.y_top, color=c.color, alpha=0.5, label=f"{c.nom}")
        ...
        ax.set_ylim(-20, H+10)
        ax.legend(loc='upper left', bbox_to_anchor=(1, 1), title="Légende des couches")
        plt.tight_layout()
        st.pyplot(fig)
        
        st.subheader("📋 Tableau Récapitulatif des Couches")
        st.table([{"Désignation": c.nom, "Cohésion (kPa)": c.c, "Frottement (°)": math.degrees(c.phi), "Poids Vol. (kN/m³)": c.gamma} for c in couches])
...
if col_btn2.button("🚀 Lancer l'Analyse Expert"):
    ...
    if erreurs:
        ...
        with st.spinner("Calcul en cours..."):
            ...
            for c in couches:
                ax.fill_between([-L, L*2], c.y_bot, c.y_top, color=c.color, alpha=0.4, label=c.nom)
            ...
            # --- AJOUT LÉGENDE GRADIENT ET AXES ---
            sm = ScalarMappable(norm=norm, cmap=cmap)
            cbar = plt.colorbar(sm, ax=ax, fraction=0.03, pad=0.15)
            ...
            st.divider()
            st.metric("Facteur de Sécurité (Optimal)", f"{best_fs:.3f}")
            st.table([{"Couche": c.nom, "Cohésion": f"{c.c} kPa", "Frottement": f"{math.degrees(c.phi)}°"} for c in couches])
